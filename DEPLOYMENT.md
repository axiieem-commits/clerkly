...
6. Run each remaining file in `supabase/migrations` in number order, through `006_remove_case_image_limit.sql`, using a new SQL Editor query for each file.
...
3. Upload several non-identifiable learning images and confirm that the page compresses and saves every selected image in one case.
